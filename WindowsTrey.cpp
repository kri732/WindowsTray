...
#define WM_COMMAND_EXTI (WM_USER + 2)
#define WM_COMMAND_NOTIFY (WM_USER + 3)
#define WM_COMMAND_HI (WM_USER + 4)
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SYSICON:
		if (lParam == WM_RBUTTONDOWN) {
			...
			AppendMenu(hMenu, MF_STRING, WM_COMMAND_HI, TEXT("hi"));
			AppendMenu(hMenu, MF_STRING, WM_COMMAND_EXTI, TEXT("Exit"));

			POINT pt;
			GetCursorPos(&pt);
			SetForegroundWindow(hWnd);
			TrackPopupMenu(hMenu, TPM_BOTTOMALIGN, TPM_LEFTALIGN, pt.x, pt.y, hWnd, NULL);
			DestroyMenu(hMenu);
	...
	case WM_COMMAND:
		...
		else if (LOWORD(wParam) == WM_COMMAND_EXTI) {
			CleanUp();
			PostQuitMessage(0);
		}
		else if (LOWORD(wParam) == WM_COMMAND_HI) {
